  ...
  for (int i = 0; i < os->cpu_count; i++){  //Inserito questa riga rispetto a originale
    ...
    if (running) {
      ...
      if (e->duration == 0){
        ...
        SchedSJFArgs* sjf_args = (SchedSJFArgs*)os->schedule_args;
        float alfa = sjf_args->alfa;
        //////////Applico la formula del SJF quando la durata dell'evento raggiunge 0 ed ho trovato la lunghezza del burst attuale 
        running->burst_stimato = (alfa * running->burst_corrente + (1-alfa) * running->burst_stimato);/////Castato a int rispetto a prima
