...
void schedSJFPREEMPTIVE(FakeOS* os, void* args_){
  ...
  for(int i = 0; i < os->cpu_count; i++){
    ...
    if (shortest_pcb) {
        printf("Processo pronto più corto (pid %d) -----> burst_stimato: %f\n", shortest_pcb->pid, shortest_pcb->burst_stimato);
    }
    else{
      printf("NON HO CORTO%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n");
    }
